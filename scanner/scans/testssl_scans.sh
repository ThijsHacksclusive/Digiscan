...
NR > 1 {
    gsub(/^"|"$/, "", $1)  # Clean ID
    gsub(/^"|"$/, "", $2)  # Clean fqdn/ip
    gsub(/^"|"$/, "", $5)  # Clean finding
...
tail -n +2 "$input_csv" | while IFS=',' read -r id target tls_suite
do
...
tail -n +2 "$OUTPUT_FILE" | while IFS=',' read -r id target tls keyex cert bulk hash; do
  ...
  target_short=$(echo $target)

  echo "$id,$target_short,$lowest" >> "$MIN_RATING_FILE"
done
